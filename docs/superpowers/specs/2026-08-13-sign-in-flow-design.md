...
- **On failed/expired code exchange:** extract just the `url` value out of `next` (regex/URLSearchParams on the decoded string) and redirect to `/diagnostic?url=<encoded>&authError=expired`. On mount, the diagnostic page sees `authError=expired` + `url` present and enters `needsSignIn` directly (not `idle`) with a `magicLinkError`-style message: "That sign-in link expired or was already used. Enter your email again to get a new one." — skipping the user having to re-paste the URL, but still requiring a fresh email submission (no session exists yet).
...
All three keep the email value in the field and offer the same retry path (`RETRY_EMAIL` → `needsSignIn`, immediately resubmittable). Per `error-handling-ux`'s prevention layer, the client-side blur validation (§3) already catches the most common "invalid email" case before it ever reaches the server.
