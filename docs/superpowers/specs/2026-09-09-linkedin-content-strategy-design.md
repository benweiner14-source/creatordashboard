...
Same numbers as Strategy Breakdown's `STRATEGY_GENERATION_PROFILE_LIMIT`/`STRATEGY_GENERATION_IP_LIMIT` — no feature-specific reason to diverge. For ideas, the real backstop is the `(profile_id, week_start)` unique constraint (one generation attempt can succeed per week); the rate limit exists to stop retry-hammering within a day, same rationale as the existing Ideas feature. A genuinely empty result (Claude honestly found nothing for the given niche/goal) does not release the slot for strategy or ideas generation, matching the established "zero output after a real attempt still counts" rule; a thrown/internal error does release it.
...
- "This week's post ideas" — its own `useEffect` fetching `GET /api/linkedin/ideas` on mount.
