...
6. **Follower-count fallback: store `null` and show "—" when a scraped platform doesn't return one.** YouTube's subscriber count is reliably available via `channels.list`; at the time of this decision, Apify's TikTok/Instagram actors were assumed to return it inconsistently. **Corrected 2026-09-18:** verified live that this wasn't inconsistency — TikTok's `authorMeta.fans` is reliably present on every profile-scrape item, while Instagram's `resultsType: "posts"` scrape (what `fetchProfilePosts` uses) never includes a follower count on any item, ever. Fixed via `fetchProfilePosts`'s new opt-in `includeFollowerCount` option (`lib/integrations/scraper.ts`), which falls back to the same `resultsType: "details"` call `fetchPost` already used for this — Watchlist now gets a real Instagram subscriber count in the normal case; `null`/"—" remains the fallback only for the rare case where even that call fails.
...
TikTok's `clockworks~tiktok-scraper` actor exposes this at `authorMeta.fans` on every item of a `resultsType: "posts"` profile scrape. Instagram's `apify~instagram-scraper` actor does **not** — a `resultsType: "posts"` scrape never includes `followersCount` on any item (verified live, 2026-09-18), contrary to this section's original assumption. Watchlist's handler calls `fetchProfilePosts` with `{ includeFollowerCount: true }`, which falls back to one extra `resultsType: "details"` call (the same one `fetchPost` already used) when no item carried a count, applying the result to every returned post. Since every item in one `fetchProfilePosts` response is scraped from the same profile, the handler still reads the follower count off the first returned post (`posts[0]?.followerCount ?? null`).
